 ...
 */
export async function registrarMovimiento(params: RegistrarMovimientoParams) : Promise<movimientos_inventario> {
  const {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (insumo_id) {
      ...
      const anterior  = Number(insumo.stock_actual);
      const posterior = tipo_movimiento === 'entrada' ? anterior + cantidad
                      : tipo_movimiento === 'salida'  ? anterior - cantidad
                      : anterior;

      if (posterior < 0)
        throw new Error(`Stock insuficiente para "${insumo.nombre}". Actual: ${anterior}`);

      await tx.insumo.update({
        where: { id: BigInt(insumo_id) },
        data:  { stock_actual: posterior.toString(), updated_at: new Date() },
      });
    }
    ...
    if (material_id) {
      ...
      const anterior  = Number(material.stock_actual);
      const posterior = tipo_movimiento === 'entrada' ? anterior + cantidad
                      : tipo_movimiento === 'salida'  ? anterior - cantidad
                      : anterior;

      if (posterior < 0)
        throw new Error(`Stock insuficiente para "${material.nombre}". Actual: ${anterior}`);
    ...
    if (producto_id) {
      ...
      if (posterior < 0)
        throw new Error(`Stock insuficiente para "${producto.nombre}". Actual: ${anterior}`);

      await tx.productos.update({
        where: { id: BigInt(producto_id) },
        data:  { stock: posterior, updated_at: new Date() },
      });
    }
    ...
    return serializeBigInt(movimiento) as movimientos_inventario;
  });
}
