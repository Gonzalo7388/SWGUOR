...
import { actualizarSeguimientoSchema } from '@/lib/schemas/seguimiento-produccion';
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  try {
    ...
    const parsed = actualizarSeguimientoSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const data = await SeguimientoProduccionService.actualizarObservaciones(
      id,
