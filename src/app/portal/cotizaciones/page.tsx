...
export default function HistorialCotizacionesPage() {
  ...
  const handleRecotizar = (cotId: number) => {
    router.push(`/portal/cotizaciones/nueva?recotizar=${cotId}`);
  };
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Link href="/portal/cotizaciones/nueva" className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-white transition-all shadow-lg bg-guor-gold hover:bg-guor-700 active:scale-95 shadow-guor-gold/30">
          <Plus size={18} /> Nueva Cotización
        </Link>
      </div>
      <CotizacionBarraFiltros filtro={filtro} setFiltro={setFiltro} busqueda={busqueda} setBusqueda={setBusqueda} />
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <CotizacionTabla cots={filtered} loading={loading} onRecotizar={handleRecotizar} />
      </div>
    </div>
  );
}