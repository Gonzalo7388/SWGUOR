      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            {canExport && (
              <>
                <Button onClick={isInsumos ? handleExportPDFInsumos : handleExportPDFMateriales} variant="outline" className="bg-white border-red-200 text-red-700 hover:bg-red-50 font-bold gap-2 h-11 active:scale-95">
                  <FileText className="w-5 h-5" /><span className="hidden sm:inline">Exportar PDF</span>
                </Button>
                <Button onClick={isInsumos ? handleExportExcelInsumos : handleExportExcelMateriales} variant="outline" className="bg-white border-emerald-200 text-emerald-700 hover:bg-emerald-50 font-bold gap-2 h-11 active:scale-95">
                  <FileSpreadsheet className="w-5 h-5" /><span className="hidden sm:inline">Exportar Excel</span>
                </Button>
              </>
            )}
            {canCreate && (
              <Button onClick={() => isInsumos ? setIsCreateInsumoOpen(true) : setIsCreateMatOpen(true)} className="bg-pink-600 hover:bg-pink-700 shadow-lg font-bold gap-2 h-11 px-6 text-white active:scale-95">
                <Plus className="w-5 h-5" />{isInsumos ? "Nuevo Insumo" : "Nuevo Material"}
              </Button>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-2 bg-white border border-gray-100 rounded-xl p-1 w-fit shadow-sm">
          <TabButton label="Insumos"    icon={<Package className="w-4 h-4" />} isActive={isInsumos}  count={statsIns.total} onClick={() => setActiveTab("insumos")} />
          <TabButton label="Materiales" icon={<Shirt   className="w-4 h-4" />} isActive={!isInsumos} count={statsMat.total} onClick={() => setActiveTab("materiales")} />
        </div>

        {/* Stats */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <StatCard title="TOTAL"         value={stats.total}    icon={<Layers           className="w-6 h-6" />} isActive={statusFilter === null}        color="pink"    onClick={() => setStatusFilter(null)} />
          <StatCard title="STOCK ÓPTIMO"  value={stats.optimo}   icon={<CheckCircle2     className="w-6 h-6" />} isActive={statusFilter === "optimo"}    color="emerald" onClick={() => setStatusFilter(statusFilter === "optimo"    ? null : "optimo")} />
          <StatCard title="STOCK BAJO"    value={stats.bajoStock} icon={<AlertTriangle   className="w-6 h-6" />} isActive={statusFilter === "bajoStock"} color="orange"  onClick={() => setStatusFilter(statusFilter === "bajoStock" ? null : "bajoStock")} />
          <StatCard title="VALOR ALMACÉN" value={`S/ ${stats.valorAlmacen.toLocaleString("es-PE", { maximumFractionDigits: 0 })}`} icon={<CircleDollarSign className="w-6 h-6" />} isActive={false} color="blue" onClick={() => {}} disabled />
        </div>

        {/* ── Toolbar (búsqueda + filtro + calculadora) ── */}
        <InventarioToolbar
          isInsumos={isInsumos}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-xl border shadow-sm">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => isInsumos ? setCurrentPageIns(p => p - 1) : setCurrentPageMat(p => p - 1)} disabled={isInsumos ? currentPageIns === 0 : currentPageMat === 0}>
                ...
                <Button variant="outline" size="sm" onClick={() => isInsumos ? setCurrentPageIns(p => p + 1) : setCurrentPageMat(p => p + 1)} disabled={isInsumos ? currentPageIns + 1 >= totalPagesIns : currentPageMat + 1 >= totalPagesMat}>
...
function TabButton({ label, icon, isActive, count, onClick }: any) {
  return (
    <button onClick={onClick} className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold transition-all ${isActive ? "bg-pink-600 text-white shadow-md" : "text-gray-600 hover:bg-gray-100"}`}>
      {icon} {label}
      <span className={`text-[11px] font-black px-2 py-0.5 rounded-full ${isActive ? "bg-white/20 text-white" : "bg-gray-200 text-gray-700"}`}>{count}</span>
    </button>
  );
}
...
function StatCard({ title, value, icon, isActive, color, onClick, disabled }: any) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} className={`group p-4 rounded-xl border transition-all duration-300 flex items-center gap-4 ${isActive ? `ring-4 shadow-xl scale-[1.02] z-10 ${s.active}` : "bg-white border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 active:scale-95 disabled:cursor-default disabled:opacity-60"}`}>
