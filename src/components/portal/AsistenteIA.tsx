...
import { Loader2, X, ArrowRight, ChevronRight, ChevronLeft } from 'lucide-react';
...
export function AsistenteIA() {
  ...
  // Si se colapsa, cerramos el chat también
  ...
  return (
    <>
      {/* ── Tab para re-expandir (visible solo cuando está colapsado) ── */}
      {collapsed && (
        <button
          onClick={handleExpand}
          title="Mostrar asistente"
          className="fixed bottom-8 right-0 z-[9999] flex items-center gap-1.5 py-3 pl-3 pr-2 rounded-l-2xl shadow-xl transition-all hover:pr-3 group"
          style={{
            background: '#1a1409',
        ...
        >
          <NexusIcon size={16} />
          <ChevronLeft size={14} color="#D4AF37" className="group-hover:-translate-x-0.5 transition-transform" />
        </button>
      )}

      {/* ── Contenedor principal de botones ── */}
      <div
        className="fixed bottom-8 right-8 z-[9999] flex flex-col items-end gap-3 transition-all duration-300"
        style={{
          transform: collapsed ? 'translateX(calc(100% + 2rem))' : 'translateX(0)',
          opacity:   collapsed ? 0 : 1,
          pointerEvents: collapsed ? 'none' : 'auto',
        }}
      >
        {/* Botón minimizar — pequeña pastilla sobre los FABs */}
        <button
          onClick={handleCollapse}
          title="Minimizar"
          className="flex items-center gap-1 px-3 py-1 rounded-full text-[10px] font-semibold transition-all hover:opacity-80"
          style={{
            background: '#1a1409',
            color: '#D4AF37',
        ...
        {/* Botón WhatsApp */}
        <a
          href="https://api.whatsapp.com/send?phone=%2B51912768800&token=eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6IjEyNSJ9.._0PaoUIvwZcBsIQR0jvdQ_i5KnVOkl9s95SY0iPkGbMDe4rVwn4vZVh1Gv69NmHUuqxGLEhJ-bzADO_WHeCeUw&fbclid=IwY2xjawRbMzJleHRuA2FlbQIxMABicmlkETFwMnFGOEhhWEdRTXh1ZjFTc3J0YwZhcHBfaWQQMjIyMDM5MTc4ODIwMDg5MgABHkM4VrxhBD8aEH3bMA8o7Mn0JTXIfFbT_C9y9kKtmRfG4vgHo3R5ouFK6c3Z_aem_JhBPXQh3IOYgFOQkiK943w"
          target="_blank"
        ...
        {isOpen && (
          <div
            className="absolute bottom-[76px] right-0 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-300"
            style={{
