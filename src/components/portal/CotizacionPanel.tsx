...
function ItemResumen({ item }: { item: ReturnType<typeof usePortal>['items'][number] }) {
  ...
  return (
    ...
    >
      ...
      {/* Cantidad (solo lectura) + subtotal */}
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Cant.</span>
          <span
            className="px-2 py-0.5 rounded text-xs font-bold bg-slate-100 text-slate-700 tabular-nums"
...
function SelectorZonaEnvio() {
  const { zonaEnvio, actualizarZonaEnvio, resumen } = usePortal();
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-auto px-3 py-3 space-y-2">
        ...
        {/* Ítems — solo lectura de cantidad */}
        {items.map(item => (
