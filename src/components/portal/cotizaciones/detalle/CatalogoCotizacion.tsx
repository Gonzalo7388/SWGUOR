...
interface ProductoBackend {
  ...
  categoria: { id: number | null; nombre: string; imagen: string | null };
}
...
export function CatalogoCotizacion({ idsAgregados }: Props) {
  ...
  const [productoModal, setProductoModal] = useState<ProductoBackend | null>(null);

  // Carga única del catálogo
  useEffect(() => {
    let active = true;
    fetch('/api/portal/productos?limite=150')
      .then(r => r.json())
      .then(({ data }) => { if (active) setProductos(data ?? []); })
      .catch(() => { if (active) setProductos([]); })
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-4 space-y-2">
        {loading ? (
          ...
          // ¿alguna variante de este producto ya está en el borrador?
          const yaAgregado = prod.variantes.some(v => idsAgregados.includes(v.id));
