...
import { Edit, ShieldOff, Eye, Building2, Mail } from "lucide-react";
...
// ─── Componente ───────────────────────────────────────────────
function ClientesTable({ usuarios, loading, onEdit, onSuspender }: Props) {
  const router      = useRouter();
  const showActions = !!onEdit || !!onSuspender;
  const colSpan     = 4 + (showActions ? 1 : 0);

  // Solo usuarios con cliente vinculado
  const clientes = usuarios.filter((u) => u.clientes != null);
  ...
  return (
    <Table>
      ...
      <TableBody>
        ...
        {/* ── Filas ── */}
        {!loading && clientes.map((user) => {
          const c      = user.clientes!;
          const activo = user.estado.toLowerCase() === "activo";
          const iniciales = (c.razon_social ?? user.email)
