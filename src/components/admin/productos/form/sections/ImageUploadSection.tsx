...
export function ImageUploadSection() {
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setUpload({ status: "error", progress: 0, message: err.message ?? "Error inesperado al subir la imagen." });
    }
  }, [setValue]);
  ...
  const handleClick = useCallback(() => {
    // Resetea el input para permitir subir el mismo archivo dos veces
    ...
    <Controller
      ...
      render={() => (
        <div className="space-y-4">
          ...
          {/* Input oculto — siempre montado para que handleClick lo encuentre */}
          <input
            ...
            >
              {upload.status === "uploading" ? (
                ...
                <div className="flex flex-col items-center gap-2 text-center px-6">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors duration-200 ${dragOver ? "bg-pink-100" : "bg-guor-peach/40"
                    }`}>
                    {dragOver
